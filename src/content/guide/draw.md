...
Decks that run Professor’s Research may have to run recovery cards, like Super Rod, to recover key components. These decks may also need to run slightly elevated copies of critical cards.

Many decks run 2-3 copies of Professor’s Research.
...
Decks that include Cynthia’s Ambition tend to run 1-2 copies of the card.
...
Decks that make use of Gardenia's Vigor tend to use 1-2 copies of the card.
...
**Serena** works well because of the huge flexibility it offers. Although not especially strong as a draw card, it does still offer a means of drawing cards when other, stronger options aren’t available to you. Whereas Boss’s Orders is a card that can’t draw you cards in an otherwise dead hand, Serena <em>can.</em> And like Boss’s Orders, Serena offers the option to gust one of your opponent’s benched Pokémon. However, Serena’s gusting power is limited to benched Pokémon V (including VSTAR, VMAX, and V-UNION.) Single prize Pokémon and Pokémon ex aren’t possible gusting targets for Serena.
...
Like Professor’s Research, **Carmine** is a very potent draw supporter in the Standard format. Unlike Professor’s Research, Carmine can be used by the player going first, even on their first turn. Like Professor’s Research, Carmine’s effect can be destructive to hands with key cards, however, its ability to draw 5 cards even on your first turn going first is unmatched in the Standard format.

Typically, only especially aggressive decks tend to run Carmine. Decks that do tend to run 2-3 copies of the card.
...
In decks that are easily shut down by a hostile Stadium cards like Lost City, Temple of Sinnoh, or Jamming Tower, Worker is an additional option for removing the hostile stadium from play.
...
**Youngster** offers a simple shuffle and draw effect that draws its player 5 cards. Unlike Iono, the player who uses Youngster is able to draw back into some or even all of the 5 cards shuffled away. This may be helpful if your deck has a number of critical 1-of or 2-of pieces that find themselves in your hand at the same time.

However, because of the popularity and superiority of Iono as a draw Supporter, Youngster sees no play.
...
Decks that run Kirlia tend to run 3-4 copies, in combination with 3-4 copies of the Basic Pokémon, Ralts.
...
**Dudunsparce’s** Run Away Draw is a powerful draw effect that also removes Dudunsparce from play by shuffling it and everything attached to it back into your deck. This makes it an effective pivoting option in addition to a draw option. Dudunsparce putting itself back into the deck can also free up crucial bench space that can be used to put other useful Pokémon into play.

Decks that run Dundunsparce tend to run 3-4 copies of the card, in addition to 3-4 copies of the Basic Pokémon, Dunsparce.
...
**Drakloak**’s Recon Directive is similar to Kirlia’s Refinement but doesn’t require any card to be discarded from your hand to use it. The tradeoff, however, is that while you still get to see the top two cards of your deck, just one of them goes to your hand and the other must be placed on the bottom of your deck.

Drakloak tends not to be run outside of decks featuring its fully evolved form, Dragapult—especially Dragapult ex. However, decks that run Drakloak tend to run 3-4 copies of the card, in addition to 3-4 copies of the Basic Pokémon, Dreepy.
...
Decks that make use of Trekking Shoes tend to include 2-4 copies of the card.
...
**Jubilife Village** is a Stadium-based draw option for decks that don’t often find themselves attacking the opponent’s Active Pokémon—especially for decks that rely on the Stall win condition to take the victory from the opponent, but don’t need to stockpile specific cards in the player’s hand. However, even decks like these may be better served by Rotom V.

Because use of Jubilife Village ends your turn when it is used, it won’t often find itself in decks. Decks that do find themselves interested in using Jubilife Village will likely run 2-3 copies of the card.
...
Decks tend not to run Gift Energy, but those that would want it tend to run 2-3 copies.
