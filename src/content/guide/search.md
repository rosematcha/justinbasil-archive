...
**Buddy-Buddy Poffin** can be an exceptional Pokémon search card if it can be consistently found and played on a player’s first turn to help their deck set up—finding two Basic Pokémon with an Item card can be a huge boon to setting a deck up for its future turns.

Buddy-Buddy Poffin is only really useful for decks that evolve from Pokémon that have 70HP or less, so decks that primarily have Basic Pokémon V or Basic Pokémon ex may not be able to make effective use of Buddy-Buddy Poffin.

Decks that run Buddy-Buddy Poffin tend to run 3-4 copies of the card in conjunction with cards that help to ensure that it can be found on a player’s first turn, like Irida or Arven.
