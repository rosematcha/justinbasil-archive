---
title: "TCG Classic"
ogImage: "/images/1d9cd7034c22b5e8.webp"
sourceUrl: "https://www.justinbasil.com/set-lists/classic"
showHeading: false
---



<h1 class="jb-center">What We Know About</h1>

<div class="jb-row"><div class="jb-col-4"></div><div class="jb-col-4">

![](/images/b0b787766c8e60b4.webp)

</div><div class="jb-col-4"></div></div>

The Pokémon Trading Card Game Classic is a self-contained game in a box featuring special holofoil versions of various cards from throughout the life of the Pokémon Trading Card Game and a handful of new cards like Ho-oh ex, Suicune ex, and Lugia ex. Pokémon Trading Card Game Classic contains three decks centered around Base Set Venusaur (CLF), Base Set Charizard (CLL), and Base Set Blastoise (CLK). Per Pokémon’s official announcement of The Pokémon Trading Card Game Classic, **no cards contained within this product will be legal for tournament play, except for the product’s basic energy cards**.

## The Decks



---



<h1 class="jb-center"><strong>Venusaur</strong></h1>



![](/images/e8d24b0586bb0829.webp)

*Illus. Mitsuhiro Arita*

<div class="jb-row"><div class="jb-col-6">

![](/images/6bb4a0399a9baa7b.webp)

</div><div class="jb-col-6">

## Deck List

**Pokémon - 19** 2 Bulbasaur BS 44 2 Ivysaur BS 30 1 Venusaur BS 15 1 Chansey BS 3 1 Doduo BKT 116 1 Dodrio UD 11 1 Hitmonchan SSH 95 1 Hitmonlee SSH 94 1 Lugia ex CLV 17 1 Onix LOT 109 1 Paras RG 72 1 Parasect SV 38 1 Pinsir SUM 6 1 Sandshrew UNB 83 1 Sandslash UNB 84 1 Scyther JU 10 1 Snorlax RCL 141

**Trainer Cards - 25** 4 Bill HS 89 4 Professor Oak CLF 26 2 Rocket's Admin. TRR 86 1 Boss's Orders RCL 154 1 Pokémon Fan Club UPR 133 1 Pokémon Nurse EX 145 2 Poké Ball SUM 125 2 Switch SVI 194 2 Ultra Ball BRS 150 1 Rare Candy SUM 129 1 Super Rod BKT 149 1 VS Seeker PHF 109 1 Sun Seed CLV 27 2 Computer Search BS 71

**Energy - 16** 8 Basic Grass Energy CLF GRA 6 Basic Fighting Energy CLF FIG 2 Double Colorless Energy BS 96

</div></div>

---



<h2 class="jb-center">Visual Card Breakdown</h2>

<div class="jb-gallery"><figure class="jb-gallery-slide"><a href="/images/fa6982bfc5f29a31.webp"><img src="/images/fa6982bfc5f29a31.webp" alt="deck-card-1-9.png" loading="lazy"></a></figure><figure class="jb-gallery-slide"><a href="/images/12ecbc43958c82bd.webp"><img src="/images/12ecbc43958c82bd.webp" alt="deck-card-1-10.png" loading="lazy"></a></figure><figure class="jb-gallery-slide"><a href="/images/ba4f6c448c3fc402.webp"><img src="/images/ba4f6c448c3fc402.webp" alt="Pokemon_Trading_Card_Game_Classic_Venusaur_png_jpgcopy.jpg" loading="lazy"></a></figure><figure class="jb-gallery-slide"><a href="/images/6f4ae748720fd87e.webp"><img src="/images/6f4ae748720fd87e.webp" alt="deck-card-1-6.png" loading="lazy"></a></figure><figure class="jb-gallery-slide"><a href="/images/a964f13c05382ee9.webp"><img src="/images/a964f13c05382ee9.webp" alt="deck-card-1-3.png" loading="lazy"></a></figure><figure class="jb-gallery-slide"><a href="/images/faa84cecfaa62893.webp"><img src="/images/faa84cecfaa62893.webp" alt="Pokemon_Trading_Card_Game_Classic_17_Lugia_ex_png_jpgcopy.jpg" loading="lazy"></a></figure><figure class="jb-gallery-slide"><a href="/images/781dbb7c162665b6.webp"><img src="/images/781dbb7c162665b6.webp" alt="deck-card-1-7.png" loading="lazy"></a></figure><figure class="jb-gallery-slide"><a href="/images/7063319d544c7dad.webp"><img src="/images/7063319d544c7dad.webp" alt="Pokemon_Trading_Card_Game_Classic_Professor_Oak_png_jpgcopy.jpg" loading="lazy"></a></figure><figure class="jb-gallery-slide"><a href="/images/35e72215021467a4.webp"><img src="/images/35e72215021467a4.webp" alt="Pokemon_Trading_Card_Game_Classic_27_Sun_Seed_png_jpgcopy.jpg" loading="lazy"></a></figure><figure class="jb-gallery-slide"><a href="/images/cfe1d72580afac62.webp"><img src="/images/cfe1d72580afac62.webp" alt="deck-card-1-8.png" loading="lazy"></a></figure></div>

---



<h2 class="jb-center">Venusaur Deck Set List</h2>

<div class="jb-code-container">
<div class="mobile-collapse links">
  <ol>
    <li class="g">Bulbasaur (Base Set)</li>
    <li class="g">Ivysaur (Base Set)</li>
    <li class="g">Venusaur (Base Set)</li>
    <li class="g">Paras (FireRed &amp; LeafGreen)</li>
    <li class="g">Parasect (Supreme Victors)</li>
    <li class="g">Scyther (Jungle)</li>
    <li class="g">Pinsir (Sun &amp; Moon)</li>
    <li class="f">Sandshrew (Unbroken Bonds)</li>
    <li class="f">Sandslash (Unbroken Bonds)</li>
    <li class="f">Onix (Lost Thunder)</li>
    <li class="f">Hitmonlee (Sword &amp; Shield)</li>
    <li class="f">Hitmonchan (Sword &amp; Shield)</li>
    <li class="c">Doduo (BREAKthrough)</li>
    <li class="c">Dodrio (Undaunted)</li>
    <li class="c">Chansey (Base Set)</li>
    <li class="c">Snorlax (Rebel Clash)</li>
    <li class="c">Lugia ex (Pokémon TCG Classic)</li>
    <li>Bill (HeartGold &amp; SoulSilver)</li>
    <li>Boss's Orders (Rebel Clash)</li>
    <li>Computer Search (Base Set)</li>
    <li>Poké Ball (Sun &amp; Moon)</li>
    <li>Pokémon Fan Club (Ultra Prism)</li>
    <li>Pokémon Nurse (Expedition)</li>
    <li>Professor Oak (Base Set)</li>
    <li>Rare Candy (Sun &amp; Moon)</li>
    <li>Rocket's Admin. (EX Team Rocket Returns)</li>
    <li>Sun Seed (Pokémon TCG Classic)</li>
    <li>Super Rod (BREAKthrough)</li>
    <li>Switch (Scarlet &amp; Violet)</li>
    <li>Ultra Ball (Brilliant Stars)</li>
    <li>VS Seeker (Phantom Forces)</li>
    <li>Basic Grass Energy</li>
    <li>Basic Fighting Energy</li>
    <li>Double Colorless Energy</li>
  </ol> 
</div>
</div>

---



<h2 class="jb-center">New Card Translations</h2>

<div class="jb-row"><div class="jb-col-2">

![](/images/faa84cecfaa62893.webp)

</div><div class="jb-col-10">

**Lugia ex**

[C][C] Rising Wind: 30 damage. Search your deck for a Basic Energy and attach it to this Pokémon. Then, shuffle your deck.

[C][C][C] Aero Break: 200 damage. This Pokémon also does 30 damage to itself.

</div></div><div class="jb-row"><div class="jb-col-2">

![](/images/35e72215021467a4.webp)

</div><div class="jb-col-10">

**Sun Seed**

The attacks by the Venusaur this card is attached to do 100 more damage to your opponent’s Active Pokémon <em>(before applying Weakness and Resistance)</em>.

</div></div>

---



<h1 class="jb-center"><strong>Charizard</strong></h1>



![](/images/61f535394fce2686.webp)

*Illus. Mitsuhiro Arita*

<div class="jb-row"><div class="jb-col-6">

![](/images/cb01f2869c2c658d.webp)

</div><div class="jb-col-6">

### Deck List

**Pokémon - 19** 2 Charmander BS 46 2 Charmeleon BS 24 1 Charizard BS 4 1 Clefairy BS 5 1 Clefable JU 1 1 Dunsparce SS 60 1 Ho-Oh ex CLC 7 1 Magmar FO 39 1 Miltank SW 55 1 Pikachu BS 58 1 Raichu SSH 66 1 Ponyta ASR 21 1 Rapidash ASR 22 1 Stantler SW 113 1 Voltorb BS 67 1 Electrode BS 21 1 Zapdos PR-SM 145

**Trainer Cards - 25** 4 Bill HS 89 4 Professor Oak CLF 26 2 Rocket's Admin. TRR 86 1 Boss's Orders RCL 154 1 Pokémon Fan Club UPR 133 2 Poké Ball SUM 125 2 Switch SVI 194 2 Ultra Ball BRS 150 1 Rare Candy SUM 129 1 Super Rod BKT 149 1 Super Scoop Up BUS 124 1 VS Seeker PHF 109 1 Scorching Charcoal CLC 26 2 Computer Search BS 71

**Energy - 16** 7 Basic Fire Energy CLL FIR 7 Basic Lightning Energy CLL LIG 2 Double Colorless Energy BS 96

</div></div>

---



<h2 class="jb-center">Visual Card Breakdown</h2>

<div class="jb-gallery"><figure class="jb-gallery-slide"><a href="/images/126001e4a84d66c2.webp"><img src="/images/126001e4a84d66c2.webp" alt="deck-card-2-9.png" loading="lazy"></a></figure><figure class="jb-gallery-slide"><a href="/images/836c6a88fb248a0a.webp"><img src="/images/836c6a88fb248a0a.webp" alt="deck-card-2-10.png" loading="lazy"></a></figure><figure class="jb-gallery-slide"><a href="/images/0ecd39edd3a897a0.webp"><img src="/images/0ecd39edd3a897a0.webp" alt="Pokemon_Trading_Card_Game_Classic_Charizard_png_jpgcopy.jpg" loading="lazy"></a></figure><figure class="jb-gallery-slide"><a href="/images/b8e7331ec64dae54.webp"><img src="/images/b8e7331ec64dae54.webp" alt="Pokemon_Trading_Card_Game_Classic_7_Ho-Oh_ex_png_jpgcopy.jpg" loading="lazy"></a></figure><figure class="jb-gallery-slide"><a href="/images/e676fec6cbea20d7.webp"><img src="/images/e676fec6cbea20d7.webp" alt="deck-card-2-3.png" loading="lazy"></a></figure><figure class="jb-gallery-slide"><a href="/images/37148e248805e9f3.webp"><img src="/images/37148e248805e9f3.webp" alt="deck-card-2-6.png" loading="lazy"></a></figure><figure class="jb-gallery-slide"><a href="/images/f3438c1287284142.webp"><img src="/images/f3438c1287284142.webp" alt="deck-card-2-4.png" loading="lazy"></a></figure><figure class="jb-gallery-slide"><a href="/images/908b3378c3b4e471.webp"><img src="/images/908b3378c3b4e471.webp" alt="Pokemon_Trading_Card_Game_Classic_26_Scorching_Charcoal_png_jpgcopy.jpg" loading="lazy"></a></figure><figure class="jb-gallery-slide"><a href="/images/7d94e5efaa567caa.webp"><img src="/images/7d94e5efaa567caa.webp" alt="deck-card-2-7.png" loading="lazy"></a></figure><figure class="jb-gallery-slide"><a href="/images/c61a2dd6d91f62e4.webp"><img src="/images/c61a2dd6d91f62e4.webp" alt="deck-card-2-8.png" loading="lazy"></a></figure></div>

---



<h2 class="jb-center">Charizard Deck Set List</h2>

<div class="jb-code-container">
<div class="mobile-collapse links">
  <ol>
    <li class="r">Charmander (Base Set)</li>
    <li class="r">Charmeleon (Base Set)</li>
    <li class="r">Charizard (Base Set)</li>
    <li class="r">Ponyta (Astral Radiance) </li>
    <li class="r">Rapidash (Astral Radiance)</li>
    <li class="r">Magmar (Fossil)</li>
    <li class="r">Ho-Oh ex (Pokémon TCG Classic)</li>
    <li class="l">Pikachu (Base Set)</li>
    <li class="l">Raichu (Sword &amp; Shield)</li>
    <li class="l">Voltorb (Base Set)</li>
    <li class="l">Electrode (Base Set)</li>
    <li class="l">Zapdos (Sun &amp; Moon Promos)</li>
    <li class="c">Clefairy (Base Set)</li>
    <li class="c">Clefable (Jungle)</li>
    <li class="c">Stantler (Secret Wonders)</li>
    <li class="c">Miltank (Secret Wonders)</li>
    <li class="c">Dunsparce (EX Sandstorm)</li>
    <li>Bill (HeartGold &amp; SoulSilver)</li>
    <li>Boss's Orders (Rebel Clash)</li>
    <li>Computer Search (Base Set)</li>
    <li>Poké Ball (Sun &amp; Moon)</li>
    <li>Pokémon Fan Club (Ultra Prism)</li>
    <li>Professor Oak (Base Set)</li>
    <li>Rare Candy (Sun &amp; Moon)</li>
    <li>Rocket's Admin. (EX Team Rocket Returns)</li>
    <li>Scorching Charcoal (Pokémon TCG Classic)</li>
    <li>Super Rod (BREAKthrough)</li>
    <li>Super Scoop Up (Burning Shadows)</li>
    <li>Switch (Scarlet &amp; Violet)</li>
    <li>Ultra Ball (Brilliant Stars)</li>
    <li>VS Seeker (Phantom Forces)</li>
    <li>Basic Fire Energy</li>
    <li>Basic Lightning Energy</li>
    <li>Double Colorless Energy</li>
  </ol> 
</div>
</div>

---



<h2 class="jb-center">New Card Translations</h2>

<div class="jb-row"><div class="jb-col-2">

![](/images/b8e7331ec64dae54.webp)

</div><div class="jb-col-10">

**Ho-Oh ex**

[R] Phoenix Breath: Attach up to 2 Basic Energy cards from your discard pile to your Pokémon in any way you like.

[R][R][C][C] Eternal Blaze: 180 damage. During your next turn, this Pokémon can’t attack.

</div></div><div class="jb-row"><div class="jb-col-2">

![](/images/908b3378c3b4e471.webp)

</div><div class="jb-col-10">

**Scorching Charcoal**

The attacks used by the Charizard this card is attached to do 100 more damage to your opponent’s Active Pokémon <em>(before applying Weakness and Resistance)</em>.

</div></div>

---



<h1 class="jb-center"><strong>Blastoise</strong></h1>



![](/images/8333b641e2566238.webp)

*Illus. Ken Sugimori*

<div class="jb-row"><div class="jb-col-6">

![](/images/b8b0e75f7e6be818.webp)

</div><div class="jb-col-6">

### Deck List

**Pokémon - 19** 2 Squirtle BS 63 2 Wartortle BS 42 1 Blastoise BS 2 1 Articuno FO 2 1 Drowzee SUM 59 1 Hypno SUM 60 1 Kangaskhan PLB 71 1 Lapras HS 24 1 Lt. Surge's Rattata G1 82 1 Lt. Surge's Raticate G1 51 1 Magikarp HS 72 1 Gyarados HS 4 1 Mewtwo EVO 51 1 Mr. Mime VEN 1 1 Staryu NXD 23 1 Starmie NXD 24 1 Suicune ex CLB 10

**Trainer Cards - 25** 4 Bill HS 89 4 Professor Oak CLF 26 2 Rocket's Admin. TRR 86 1 Boss's Orders RCL 154 1 Fisherman BKT 136 1 Pokémon Fan Club UPR 133 2 Poké Ball SUM 125 2 Switch SVI 194 2 Ultra Ball BRS 150 1 Rare Candy SUM 129 1 Super Rod BKT 149 1 VS Seeker PHF 109 1 Drops in the Ocean CLB 21 2 Computer Search BS 71

**Energy - 16** 8 Basic Water Energy CLK WAT 6 Basic Psychic Energy CLK PSY 2 Double Colorless Energy BS 96

</div></div>

---



<h2 class="jb-center">Visual Card Breakdown</h2>

<div class="jb-gallery"><figure class="jb-gallery-slide"><a href="/images/0c3fd09e662212fb.webp"><img src="/images/0c3fd09e662212fb.webp" alt="deck-card-3-9.png" loading="lazy"></a></figure><figure class="jb-gallery-slide"><a href="/images/8fc43852246f2ef0.webp"><img src="/images/8fc43852246f2ef0.webp" alt="deck-card-3-10.png" loading="lazy"></a></figure><figure class="jb-gallery-slide"><a href="/images/c982f63a1a6fb4e9.webp"><img src="/images/c982f63a1a6fb4e9.webp" alt="Pokemon_Trading_Card_Game_Classic_Blastoise_png_jpgcopy.jpg" loading="lazy"></a></figure><figure class="jb-gallery-slide"><a href="/images/dbd8154396551cd5.webp"><img src="/images/dbd8154396551cd5.webp" alt="Pokemon_Trading_Card_Game_Classic_10_Suicune_ex_png_jpgcopy.jpg" loading="lazy"></a></figure><figure class="jb-gallery-slide"><a href="/images/fd74966e99738419.webp"><img src="/images/fd74966e99738419.webp" alt="deck-card-3-3.png" loading="lazy"></a></figure><figure class="jb-gallery-slide"><a href="/images/4adbf4b0c97db621.webp"><img src="/images/4adbf4b0c97db621.webp" alt="deck-card-3-6.png" loading="lazy"></a></figure><figure class="jb-gallery-slide"><a href="/images/a60875fdd8c74d21.webp"><img src="/images/a60875fdd8c74d21.webp" alt="deck-card-3-7.png" loading="lazy"></a></figure><figure class="jb-gallery-slide"><a href="/images/85b3caa3a778420d.webp"><img src="/images/85b3caa3a778420d.webp" alt="Pokemon_Trading_Card_Game_Classic_21_Drops_in_the_Ocean_png_jpgcopy.jpg" loading="lazy"></a></figure><figure class="jb-gallery-slide"><a href="/images/d73b57a18c87c4ca.webp"><img src="/images/d73b57a18c87c4ca.webp" alt="deck-card-3-5.png" loading="lazy"></a></figure><figure class="jb-gallery-slide"><a href="/images/c2c1530a1e94ed0c.webp"><img src="/images/c2c1530a1e94ed0c.webp" alt="deck-card-3-8.png" loading="lazy"></a></figure></div>

---



<h2 class="jb-center">Blastoise Deck Set List</h2>

<div class="jb-code-container">
<div class="mobile-collapse links">
  <ol>
    <li class="w">Squirtle (Base Set)</li>
    <li class="w">Wartortle (Base Set)</li>
    <li class="w">Blastoise (Base Set)</li>
    <li class="w">Staryu (Next Destinies) </li>
    <li class="w">Starmie (Next Destinies)</li>
    <li class="w">Magikarp (HeartGold &amp; SoulSilver)</li>
    <li class="w">Gyarados (HeartGold &amp; SoulSilver)</li>
    <li class="w">Lapras (HeartGold &amp; SoulSilver)</li>
    <li class="w">Articuno (Fossil)</li>
    <li class="w">Suicune ex (Pokémon TCG Classic)</li>
    <li class="p">Drowzee (Sun &amp; Moon)</li>
    <li class="p">Hypno (Sun &amp; Moon)</li>
    <li class="p">Mr. Mime (Vending)</li>
    <li class="p">Mewtwo (Evolutions)</li>
    <li class="c">Lt. Surge's Rattata (Gym Heroes)</li>
    <li class="c">Lt. Surge's Raticate (Gym Heroes)</li>
    <li class="c">Kangaskahn (Plasma Blast)</li>
    <li>Bill (HeartGold &amp; SoulSilver)</li>
    <li>Boss's Orders (Rebel Clash)</li>
    <li>Computer Search (Base Set)</li>
    <li>Drops in the Ocean (Pokémon TCG Classic)</li>
    <li>Fisherman (BREAKthrough)</li>
    <li>Poké Ball (Sun &amp; Moon)</li>
    <li>Pokémon Fan Club (Ultra Prism)</li>
    <li>Professor Oak (Base Set)</li>
    <li>Rare Candy (Sun &amp; Moon)</li>
    <li>Rocket's Admin. (EX Team Rocket Returns)</li>
    <li>Super Rod (BREAKthrough)</li>
    <li>Switch (Scarlet &amp; Violet)</li>
    <li>Ultra Ball (Brilliant Stars)</li>
    <li>VS Seeker (Phantom Forces)</li>
    <li>Basic Water Energy</li>
    <li>Basic Psychic Energy</li>
    <li>Double Colorless Energy</li>
  </ol> 
</div>
</div>

---



<h2 class="jb-center">New Card Translations</h2>

<div class="jb-row"><div class="jb-col-2">

![](/images/dbd8154396551cd5.webp)

</div><div class="jb-col-10">

**Suicune ex**

Ability: Aqua Lift If this Pokémon has any [W] Energy attached to it, it has no Retreat Cost.

[C][C][C] Crystal Blast: 60+ damage. This attack does 30 more damage for each [W] Energy attached to this Pokémon.

</div></div><div class="jb-row"><div class="jb-col-2">

![](/images/85b3caa3a778420d.webp)

</div><div class="jb-col-10">

**Drops in the Ocean**

The attacks used by the Blastoise this card is attached to do 100 more damage to your opponent’s Active Pokémon <em>(before applying Weakness and Resistance)</em>.

</div></div>

---



